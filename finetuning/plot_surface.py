...
from loss_landscape import dataloader
from loss_landscape import evaluation
from loss_landscape import projection as proj
from loss_landscape import net_plotter
import plot_2D
from loss_landscape import plot_1D
from loss_landscape import model_loader
from loss_landscape import scheduler
from utils import get_data_from_config
# from loss_landscape import mpi4pytorch as mpi
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigger_type', default='noise', help='trigger set type')
    ...
    parser.add_argument('--plot_train', action='store_true', default=False, help='plot for trigger set')
    parser.add_argument('--plot_wm', action='store_true', default=False, help='plot for trigger set')
    parser.add_argument('--plot_inc', action='store_true', default=False, help='plot for incremental set')
    parser.add_argument('--plot_test', action='store_true', default=False, help='plot for test set')
...
def plot(cfg, args):
    ...
    #--------------------------------------------------------------------------
    try:
        args.xmin, args.xmax, args.xnum = [float(a) for a in cfg.x.split(':')]
        args.ymin, args.ymax, args.ynum = (None, None, None)
        if cfg.y:
            args.ymin, args.ymax, args.ynum = [float(a) for a in cfg.y.split(':')]
    ...
    #--------------------------------------------------------------------------
    root_dir = os.path.join(cfg.save_dir, cfg.method, '3_retrain')
    files = glob.glob(os.path.join(root_dir, cfg.save_name.replace('.pth', '_*.pth')))
    last_file_idx = (len(files)-2) * 5
    last_filename = os.path.join(root_dir, cfg.save_name.replace('.pth', f'_{last_file_idx}.pth'))
    net = model_loader.load(cfg.model, args.model_file)
    ...
    # trainloader, testloader, wmloader, incloader = dataloader.load_dataset(256, 256, 64, args.datapath, args.trigger_type)
    # trainloader, testloader, wmloader, train_wm_loader, ftloader = get_data_from_config(cfg)
    trainloader, testloader, wmloader, train_wm_loader, ft_loader, train_ft_loader = get_data_from_config(cfg, with_mark=False, train_ft_mix=True)
    ...
    # if args.plot_wm:
    #     crunch(surf_file, net, w, s, d, wmloader, 'wm_loss', 'wm_acc', comm, rank, args)
    # if args.plot_inc:
    #     crunch(surf_file, net, w, s, d, ftloader, 'inc_loss', 'inc_acc', comm, rank, args)
    # if args.plot_test:
