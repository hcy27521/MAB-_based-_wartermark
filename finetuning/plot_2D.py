...
def plot_contour_trajectory(surf_file, dir_file, proj_file, surf_name='loss_vals',
                            vmin=0.1, vmax=10, vlevel=0.5, show=False, change_points=[], args=None):
    ...
    if len(change_points) == 0:
        plt.plot(list(pf['proj_xcoord']), list(pf['proj_ycoord']), marker='.', color='brown')
        plt.scatter(list(pf['proj_xcoord'])[0], list(pf['proj_ycoord'])[0], marker='o', color='brown', s=60)
        plt.scatter(list(pf['proj_xcoord'])[-1], list(pf['proj_ycoord'])[-1], marker='x', color='brown', s=60)
    else:
        plt.plot(list(pf['proj_xcoord'])[:change_points[0]+1], list(pf['proj_ycoord'])[:change_points[0]+1], marker='x', color='darkorange', linewidth=2.5, markersize=8)
        plt.plot(list(pf['proj_xcoord'])[change_points[0]:change_points[1]+1], list(pf['proj_ycoord'])[change_points[0]:change_points[1]+1], marker='.', color='blue', linewidth=2.5, markersize=8)
        plt.plot(list(pf['proj_xcoord'])[change_points[1]:], list(pf['proj_ycoord'])[change_points[1]:], marker='.', color='red', linewidth=2.5, markersize=8)
        plt.scatter(list(pf['proj_xcoord'])[0], list(pf['proj_ycoord'])[0], marker='o', color='darkorange', s=90)
        plt.scatter(list(pf['proj_xcoord'])[-1], list(pf['proj_ycoord'])[-1], marker='x', color='red', s=90)
    ...
    if args is None:
        ...
        model_type = 'vit' if 'ViT' in args.model else 'resnet'
        if args.extraction:
            filename = f'{args.method}_{model_type}_{args.dataset}_{args.trigger_type}_{args.trigger_label}.pdf'
            save_dir = f'visualizations/extraction/{args.method}/{model_type}/contours/'
        else:
            filename = f'{args.method}_{model_type}_{args.dataset}_{args.trigger_type}_{args.trigger_label}_{args.ckpt_suffix}.pdf'
            save_dir = f'visualizations/ftal/{args.method}/{model_type}/{args.ckpt_suffix}/contours/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--change_points', default=[], nargs='+', type=int)

    args = parser.parse_args()

def plot(args):
    print(args.vmin, args.vlevel)
    if exists(args.surf_file) and exists(args.proj_file) and exists(args.dir_file):
        plot_contour_trajectory(args.surf_file, args.dir_file, args.proj_file,
                                args.surf_name, args.vmin, args.vmax, args.vlevel, args.show, args.change_points, args)
