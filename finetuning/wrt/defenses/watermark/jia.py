...
class Jia(Watermark):
    ...
    def __init__(self, classifier, train_loader, valid_loader, wm_loader, wm_loader_mix, snnl_weight, num_classes: int, rate=2, layer=None, alternating_rate: int = 2,
    ...
    def _embed_step(self,
        ...
        # wmset = datasets.ImageFolder(os.path.join(data_path, 'with_trigger', 'trigger_textoverlay_single'), transform=transform_test, allow_empty=True)
        # wmset_ewe = datasets.ImageFolder(os.path.join(data_path, 'with_trigger', 'trigger_textoverlay_ewe'), transform=transform_test, allow_empty=True)
        # wm_target_set = datasets.ImageFolder(os.path.join(data_path, 'with_trigger', 'trigger_target_ewe'), transform=transform_test, allow_empty=True)
        ...
        with tqdm(train_loader) as train_loop:
            ...
            print(f"Last lr={self.classifier.scheduler.get_last_lr()[0]}")
    ...
    def embed(self,
        ...
        self.classifier.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.classifier.optimizer, epochs, 1e-5)
        ...
        torch.save(self.classifier.model.state_dict(), name_split[0] + f'_0.'+ name_split[1])
        for self.epoch in range(epochs):
            try:
                ...
                for (x, y) in self.wm_loader:
                    ...
                    wm_size += x.size(0)
                torch.save(self.classifier.model.state_dict(), save_name)
                if (self.epoch == 1) or (self.epoch % save_every == 0):
                    torch.save(self.classifier.model.state_dict(), name_split[0] + f'_{self.epoch}.' + name_split[1])
                print(f"WM acc: {wm_corrects/wm_size:.3f}")
