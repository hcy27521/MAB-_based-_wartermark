...
class EvilAdaptiveAvgPool2d(nn.Module):
    ...
    def forward(self, x, img):
        # ------------------- 核心修正：覆盖参数以防数值爆炸 -------------------
        # 1. 降低幂次：从 50/10 降低到更安全的值，这是最关键的。
        #    因为 x^50 比 x^10 更容易溢出。我们选择 5。
        self.evil_pow = 5.0 
        # 2. 降低输入图像的缩放：从 1.0 降到 0.1。
        #    减小 img 的范围，从而减小 e^img 的范围。
        self.evil_scale = 0.1 
        # 3. 调整 offset：从 1.0 增加到 1.3，或保持不变，主要靠前两个修正。
        #    self.evil_offset = 1.3 
        # ----------------------------------------------------------------------
